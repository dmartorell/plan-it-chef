import { React, useState } from 'react';
import { useSelector, useDispatch } from 'react-redux';
import { Link } from 'react-router-dom';
import { getFormattedRecipe, loadRecipes } from '../../redux/actions/actionCreators';
import './style.scss';
import logo from '../../assets/logo-sm.png';

const Header = () => {
  const userName = useSelector((store) => store.user.firstName);
  const dispatch = useDispatch();
  const token = useSelector((store) => store.user.token);
  const [inputValue, setInputValue] = useState('');

  const handleClick = async () => {
    setInputValue('');
    await dispatch(getFormattedRecipe(inputValue, token));
    await dispatch(loadRecipes(token));
  };

  return (
    <header className="header">
      <div className="header__top">
        <figure>
          <Link to="/recipes">
            <img className="logo" src={logo} alt="logo" data-testid="logo-sm" />
          </Link>
        </figure>
        <button className="logout-btn" type="button" data-testId="refreshPage">
          <div className="user-info">
            <p className="user-info__name">{userName}</p>
            <span className="iconify user-info__icon" data-icon="carbon:user-avatar-filled" data-inline="false" />
          </div>
        </button>
      </div>
      <div className="header__bottom">
        <form className="header-form">
          <input type="text" className="form-input" placeholder="Paste recipe url..." value={inputValue} onChange={(event) => { setInputValue(event.target.value); }} autoComplete="off" data-testId="recipeUrl" />
          <button type="button" className="form-btn" onClick={handleClick} data-testId="getJson">
            <img className="form-btn__icon" src="data:image/png;base64," alt="icon" />
          </button>
        </form>
      </div>
    </header>
  );
};

export default Header;
